...
// Meta Llama 4 Scout (17B) Vision — OpenRouter üzerinden
async function callLlama4Vision(imageB64, imageMime) {
  ...
  const d = await res.json();
  if (!res.ok) throw new Error(`Llama 4 Vision: ${d.error?.message || res.statusText}`);
...
app.post('/api/read-image', wrap(async (req, res) => {
  ...
  const text = await callLlama4Vision(imageB64, imageMime);
...
app.post('/api/classify', wrap(async (req, res) => {
  ...
  // Modelinin eklediği açıklama/markdown ne olursa olsun { } bloğunu yakala
...
initOpenRouterModels().then(() => {
  app.listen(PORT, () => {
    ...
    console.log(`  ➜ Vizyon  : Meta Llama 4 Scout 17B`);
